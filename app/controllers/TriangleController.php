...
class TriangleController
{
    ...
    public function index()
    {
        ...
        if ($result) {
            ...
            $triangulos = $result->fetchAll(\PDO::FETCH_ASSOC);

            // Encode the result as JSON and output it
            header('Content-Type: application/json');
            echo json_encode($triangulos, JSON_PRETTY_PRINT);
        } else {
    ...
    public function store()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($width !== null && $length !== null) {
                ...
                $insertSql = "INSERT INTO triangulos (width, length, area) VALUES ('$width', '$length', '$area')";
    ...
    public function destroy()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
            ...
            if ($id !== null) {
                ...
                $deleteSql = "DELETE FROM triangulos WHERE id = '$id'";
                $deleteResult = $conn->exec($deleteSql);

                if ($deleteResult !== false) {
                    echo "O triangulo com ID:$id foi  excluído com sucesso!";
                } else {
